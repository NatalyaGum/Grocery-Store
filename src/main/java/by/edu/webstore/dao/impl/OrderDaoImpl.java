...
public class OrderDaoImpl implements OrderDao {
    ...
    private static final String ADD_PRODUCT_TO_ORDER = "INSERT INTO purchases (id_order, id_product, quantity) VALUE (?, ?, ?)";
    ...
    private static final String FIND_ORDERS_BY_USER_ID_PAGES = """
            ...
            FROM orders
            JOIN  addresses ON addresses.id_address=orders.delivery_address_id 
            JOIN users ON orders.users_user_id=users.id_user  ORDER BY id_order DESC
            WHERE orders.users_user_id=?  LIMIT ?,?""";
    private static final String FIND_PRODUCTS_OF_ORDER = "SELECT id_product, quantity FROM purchases WHERE id_order=?";
    private static final String FIND_TOTAL_ORDERS_NUMBER ="SELECT COUNT(id_order) FROM orders WHERE users_user_id=?";
        ...
        try {
            ...
            try (PreparedStatement orderStatement = connection.prepareStatement(INSERT_NEW_ORDER, RETURN_GENERATED_KEYS);
              ...
              //  orderStatement.setString(4, order.getMethod().toString());
               orderStatement.setLong(5, order.getUserId());
    ...
    public List<Order> findAllOrdersOfUser(long user_id,int offset, int limit) throws DaoException {
        try (Connection connection = connectionPool.getConnection();
            ...
            while (resultSet.next()) {
                long order_id=resultSet.getLong(ColumnName.ORDER_ID);
                PreparedStatement statementProducts = connection.prepareStatement(FIND_PRODUCTS_OF_ORDER);
                    ...
                    ResultSet resultSetProductId = statement.executeQuery();
    ...
    public int findTotalOrdersNumber (long user_id) throws DaoException {
        int result = 0;
        try (Connection connection = connectionPool.getConnection();
             PreparedStatement statement = connection.prepareStatement(FIND_TOTAL_ORDERS_NUMBER)) {
            statement.setLong(1, user_id);
            ResultSet resultSet = statement.executeQuery();
            int  ordersNumber = 0;
            while (resultSet.next()) {
